    ...
    public static void fromJsonToCollection() {
        if (filePath != null) {
            try (Reader reader = new FileReader(filePath)) {
                ...
                List<Product> Products = gson.fromJson(reader, new TypeToken<List<Product>>() {
                }.getType());
                if (Products.size() > 0) for (Product product : Products) {
                    CollectionManager.addJsonObject(product);
                }
